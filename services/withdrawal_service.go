...
func (s *withdrawalService) Create(createdByID uint, req transactiondto.CreateRequest) (*models.Withdrawal, error) {
	...
	clientBankName := "—"
	if result.ClientBank != nil {
		if result.ClientBank.BankType != nil {
			clientBankName = result.ClientBank.BankType.Name + " - " + result.ClientBank.AccountNo
		} else {
			clientBankName = result.ClientBank.AccountNo
		}
	}
	productName := "—"
	if result.ClientProduct != nil && result.ClientProduct.ProductType != nil {
		productName = result.ClientProduct.ProductType.Name
	}
	createdByName := "—"
	...
	notifyTelegramForBranch(s.branchRepo, req.BranchID, true, fmt.Sprintf(
		"💸 <b>Withdrawal</b>\nClient: %s\nClient Bank: %s\nCompany Bank: %s\nProduct: %s\nPlay: %.2f\nTxn: %s\nAmount: %.2f %s\nBy: %s",
		clientCode, clientBankName, bankName, productName, req.Play, txNo, req.Amount, currency, createdByName,
	))
